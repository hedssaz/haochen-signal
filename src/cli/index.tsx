...
function toolDefinitions(): Map<string, ToolDefinitionSpec<unknown, unknown>> {
  const specs: ToolDefinitionSpec<unknown, unknown>[] = [
    ...
    {name: 'read_file', description: '读取工作区文本文件（支持按字符续读）', inputSchema: z.object({path: z.string(), startLine: z.number().int().optional(), endLine: z.number().int().optional(), startCharacter: z.number().int().min(0).optional(), maxCharacters: z.number().int().min(1).max(65_536).optional()}).strict(), jsonSchema: objectSchema({path: {type: 'string'}, startLine: {type: 'integer'}, endLine: {type: 'integer'}, startCharacter: {type: 'integer'}, maxCharacters: {type: 'integer'}}, ['path']), execute: (i, c, s) => readFileTool(i as {path: string; startLine?: number; endLine?: number; startCharacter?: number; maxCharacters?: number}, c, s)},
